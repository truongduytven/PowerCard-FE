...
export const FlashcardCard: React.FC<FlashcardCardProps> = ({
  ...
  // Reset touched state khi card thay đổi
  useEffect(() => {
    if (!isExpanded) {
      setHasBeenTouched({ term: false, definition: false });
    }
  }, [isExpanded]);

  // Track khi người dùng bắt đầu nhập vào field
  const handleTermChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    if (!hasBeenTouched.term) {
      setHasBeenTouched((prev) => ({ ...prev, term: true }));
    }
    onChange(index, "term", e.target.value);
  };

  const handleDefinitionChange = (
    e: React.ChangeEvent<HTMLTextAreaElement>,
  ) => {
    if (!hasBeenTouched.definition) {
      setHasBeenTouched((prev) => ({ ...prev, definition: true }));
    }
    onChange(index, "definition", e.target.value);
  };

  // Chỉ hiển thị lỗi khi field đã được chạm vào và trống
  const showTermError = hasBeenTouched.term && !card.term.trim();
  const showDefinitionError =
    hasBeenTouched.definition && !card.definition.trim();
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
              ...
              <button
                ...
                className="opacity-100 transition-opacity duration-200 p-1 sm:p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 dark:hover:text-red-400 rounded-md text-gray-400 dark:text-gray-500"
                title="Xóa thẻ này"
              >
              ...
              <div className="lg:col-span-5 space-y-2">
                ...
                <Textarea
                  ...
                  className={`min-h-[100px] sm:min-h-[120px] resize-none text-sm rounded-lg bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 text-gray-900 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-400 ${
                    showTermError ? "border-red-300 dark:border-red-500" : ""
                  }`}
                />
              </div>
